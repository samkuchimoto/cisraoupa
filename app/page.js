...
// ============================================================
// MASCOT SVG — Yellow chaotic creature, multiple moods
// ============================================================
function Mascot({ mood = 'default', size = 80 }) {
  const moods = {
    default: { eyeL: '😐', eyeR: '😐', mouth: '—', bounce: true },
    roasting: '🔥',
    dead: '💀',
    shocked: '😱',
    nodding: '😤',
...
// ============================================================
// FOMO COUNTER — viral social proof
// ============================================================
function FomoCounter() {
  ...
  useEffect(() => {
    ...
    supabase.from('cisraoupa_roasts').select('*', { count: 'exact', head: true })
      .then(({ count: c }) => {
        // Add base offset so it always looks busy (virality trick)
        setCount((c || 0) + 847)
      })
    // Live increment for drama
...
// ============================================================
// TIMER BAR — burns down during rounds
// ============================================================
...
function LoadingScreen() {
  const loadingLines = ['Analysing your cope levels...','Calibrating the roast cannon...','Consulting the savage oracle...','Calculating how cooked you are...']
  const [lineIdx, setLineIdx] = useState(0)
  useEffect(() => {
    const interval = setInterval(() => setLineIdx(i => (i + 1) % loadingLines.length), 1200)
    return () => clearInterval(interval)
  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center px-4">
      <div className="text-center">
        <div className="mascot-roast text-8xl mb-6">🔥</div>
        <div className="flex justify-center gap-2 mb-6">
          {[0,1,2].map(i => <div key={i} className="load-dot w-3 h-3 rounded-full" style={{background:'#FFE600'}} />)}
        </div>
        <p className="text-lg font-black text-yellow-400 uppercase tracking-tight" key={lineIdx} style={{animation:'slideUp 0.3s ease-out'}}>{loadingLines[lineIdx]}</p>
        <p className="text-xs text-gray-600 mt-2 font-bold">This takes ~5 seconds. Brace yourself.</p>
      </div>
    </main>
  )
}
...
export default function CisraoupaApp() {
  const [screen, setScreen] = useState('landing') // landing | input | round | loading | result | email_done
  ...
  const [shakeChoice, setShakeChoice] = useState(null)
    ...
    timerRef.current = setInterval(() => {
      setTimeLeft(prev => {
        if (prev <= 1) {
          ...
          // Auto-advance on timeout (pick 'wait' as default — funnier roast)
  ...
  async function callAnalyze(finalAnswers) {
    try {
      ...
      setResult({
        ...
        verdict: "Classic situation: real obstacles + manufactured hesitation. The system does block people. And so does comfortable suffering.",
  ...
  async function handleEmailSubmit() {
    ...
    try {
      ...
      setEmailSent(true) // Still show success — don't block UX on email failure
    }
    ...
    return (
      <main className="min-h-screen dot-bg flex flex-col items-center justify-center px-4 relative overflow-hidden">
        ...
        <div className="relative z-10 text-center max-w-lg mx-auto">
          {/* FOMO counter — social proof before anything */}
          <div className="mb-6">
            <FomoCounter />
          </div>
          ...
          {/* Hook line */}
          <p className="text-xl md:text-2xl font-black text-white mb-1 uppercase tracking-tight">
            Drop your problem.
          </p>
          <p className="text-xl md:text-2xl font-black mb-6 uppercase tracking-tight" style={{ color: '#FF3B3B' }}>
            Get destroyed. Get fixed.
          </p>
          ...
          {/* Watermark — viral signal */}
          <div className="mt-12 text-xs text-gray-700 font-black uppercase tracking-widest">
            Cisraoupa · Built different
          </div>
        </div>
      </main>
    )
      ...
      <main className="min-h-screen flex flex-col items-center justify-center px-4 py-12">
        <div className="w-full max-w-lg">
          ...
          <div className="text-center mb-8 slide-up">
            ...
            <h2 className="text-3xl font-black mt-3 uppercase tracking-tight" style={{ color: '#FFE600' }}>
              What's the situation?
            </h2>
            <p className="text-gray-400 text-sm mt-2 font-bold" style={{ fontFamily: 'Arial, sans-serif' }}>
              Career, company drama, life decision — anything. Be real.
            </p>
          </div>
          ...
          <div className="slide-up slide-up-delay-1">
            ...
            <div className="flex justify-between mt-1 text-xs text-gray-600 font-bold">
              ...
              <span style={{ color: situation.length > 350 ? '#FF3B3B' : '#555' }}>
                {situation.length}/400
              </span>
            </div>
          </div>
          ...
          <div className="mt-4 slide-up slide-up-delay-2">
            ...
            <div className="flex flex-wrap gap-2">
              {[
                ...
                'Starting my own thing vs stable job',
                'Company keeps promoting less qualified people',
                'Burnout but golden handcuffs',
              ].map(ex => (
                <button
                  key={ex}
          ...
          <button onClick={restart} className="mt-4 w-full text-xs text-gray-600 hover:text-gray-400 font-bold py-2">
            ← back
          </button>
        </div>
      </main>
        ...
        <div className="w-full max-w-lg">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <div className="text-gray-400 text-sm font-bold" style={{ fontFamily: 'Arial, sans-serif' }}>
                {round.subtitle}
              </div>
            </div>
            <div className="text-right">
              <div
                ...
                style={{ color: timeLeft <= 5 ? '#FF3B3B' : '#FFE600' }}
              >
                {timeLeft}s
              </div>
          ...
          {/* Timer bar */}
          <div className="mb-6">
            <TimerBar seconds={timeLeft} total={ROUND_TIME} />
          </div>
          ...
          <div className="flex gap-2 mb-6">
            {[1, 2, 3].map(r => (
              <div
                ...
                style={{ background: r <= currentRound ? '#FFE600' : '#2a2a2a' }}
              />
            ))}
          </div>
          ...
          <div className="space-y-3">
            {round.choices.map((choice) => (
              <button
                ...
                className={`choice-btn w-full text-left px-5 py-4 rounded-xl font-black text-base transition-all
                  ${selectedChoice === choice.value ? 'border-yellow-400 bg-yellow-400 text-black' : 'text-white'}
                `}
                style={{
              ...
              >
                <span
                  className="inline-block w-7 h-7 rounded-md text-center text-sm font-black mr-3"
                  style={{
          ...
          <p
            className="mt-4 text-center text-xs text-gray-700 font-bold"
        ...
        <div className="text-center">
          ...
          <div className="flex justify-center gap-2 mb-6">
            {[0, 1, 2].map(i => (
              <div
                key={i}
                className="load-dot w-3 h-3 rounded-full"
    ...
    return (
      <main className="min-h-screen flex flex-col items-center px-4 py-10">
        <div className="w-full max-w-lg">
          ...
          {/* Score — the moment */}
          <div
            className="rounded-2xl p-8 mb-4 text-center border-2 slide-up"
            style={{ background: '#1a1a1a', borderColor: '#FFE600' }}
          >
            <div className="text-3xl mb-3">
              <Mascot mood={result.mascot_reaction || 'roasting'} size={56} />
            </div>
            <div className="text-xs font-black uppercase tracking-widest text-gray-500 mb-2">
              YOUR ROAST SCORE
            </div>
            <div
              className="score-pop text-8xl font-black leading-none mb-1"
            ...
            <div
              className="text-sm font-black uppercase tracking-widest px-4 py-1 rounded-full inline-block"
          ...
          {/* Roast line — the money shot */}
          <div
            className="rounded-2xl p-6 mb-4 border-l-4 slide-up slide-up-delay-1"
            style={{ background: '#111', borderColor: '#FF3B3B' }}
          >
            <div className="text-xs font-black uppercase tracking-widest mb-2" style={{ color: '#FF3B3B' }}>
              💀 THE ROAST
            </div>
          ...
          {/* Verdict */}
          <div
            className="rounded-2xl p-6 mb-4 slide-up slide-up-delay-2"
            style={{ background: '#1a1a1a', border: '1px solid #2a2a2a' }}
          >
            <div className="text-xs font-black uppercase tracking-widest mb-3 text-gray-500">
              🔍 THE REAL DEAL
            </div>
            <p className="text-sm text-gray-300 leading-relaxed font-bold" style={{ fontFamily: 'Arial, sans-serif' }}>
              {result.verdict}
            </p>
          </div>
          ...
          {/* Solution */}
          <div
            className="rounded-2xl p-6 mb-6 border-l-4 slide-up slide-up-delay-3"
            style={{ background: '#0d1a14', borderColor: '#00FF88' }}
          >
            <div className="text-xs font-black uppercase tracking-widest mb-3" style={{ color: '#00FF88' }}>
              ⚡ THE FIX (actually)
            </div>
            <p
              className="text-sm text-gray-200 leading-relaxed whitespace-pre-line font-bold"
              style={{ fontFamily: 'Arial, sans-serif' }}
            >
              {result.solution}
            </p>
          ...
          {/* SHARE — primary virality action */}
          <div className="mb-4 slide-up slide-up-delay-4">
            <button
              onClick={handleShare}
              className="share-pulse w-full font-black text-lg py-5 rounded-xl uppercase tracking-tight text-black"
              style={{ background: '#FFE600' }}
            >
              🐦 SHARE YOUR ROAST (flex on them)
            </button>
            <p className="text-center text-xs text-gray-600 mt-2 font-bold" style={{ fontFamily: 'Arial, sans-serif' }}>
              Pre-filled tweet. One tap. Maximum damage.
            </p>
          </div>

          {/* Email capture — placed AFTER value delivered */}
          {!emailSent ? (
            <div
            ...
            >
              <p className="text-sm font-black text-white mb-1">
                Get the full roast + solution in your inbox
              </p>
              <p className="text-xs text-gray-500 mb-4 font-bold" style={{ fontFamily: 'Arial, sans-serif' }}>
                No spam. Just this one email. Unsubscribe anytime.
              </p>
            ...
            <div
              className="rounded-2xl p-4 mb-4 text-center slide-up"
              style={{ background: '#0d1a14', border: '1px solid #00FF88' }}
            >
              <p className="font-black text-sm" style={{ color: '#00FF88' }}>
                ✅ Roast report incoming. Check your inbox.
              </p>
            </div>
          )}
          ...
          <div className="text-center">
            <button
              onClick={restart}
              className="text-sm text-gray-600 hover:text-yellow-400 font-black uppercase tracking-widest transition-colors"
            >
              🔄 ROAST SOMEONE ELSE'S PROBLEM
            </button>
          </div>

          {/* Watermark — viral brand signal */}
          <div className="text-center mt-6 text-xs text-gray-700 font-black uppercase tracking-widest">
            Cisraoupa roasted me 😂 · cisraoupa.com
          </div>
        </div>
      </main>
    )
